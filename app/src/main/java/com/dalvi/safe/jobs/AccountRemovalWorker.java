...
public class AccountRemovalWorker extends Worker {
    ...
    public Result doWork() {
        Objects.requireNonNull(SafeAppApplication.Companion.getSharedApplication()).getComponentApplication().inject(this);
        ...
        for (User user : users) {
            if (user.getPushConfigurationState() != null) {
                ...
                ncApi.unregisterDeviceForNotificationsWithSafe App(
                        ApiUtils.getCredentials(user.getUsername(), user.getToken()),
                        ApiUtils.getUrlSafe AppPush(Objects.requireNonNull(user.getBaseUrl())))
